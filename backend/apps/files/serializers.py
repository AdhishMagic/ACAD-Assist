...
from .services import MAX_UPLOAD_SIZE_BYTES, build_upload_metadata, compute_checksum, detect_file_type, extract_editable_content
...
class FileUploadSerializer(serializers.ModelSerializer):
	...
	def create(self, validated_data):
		...
		content = extract_editable_content(uploaded_file, file_type) if file_type in {"docx", "txt"} else ""

		instance = File.objects.create(
			file=uploaded_file,
			file_type=file_type,
			uploaded_by=self.context["request"].user,
			is_editable=file_type in {"docx", "txt"},
	...
	def get_extracted_content(self, obj):
		return getattr(self, "_editable_content", "") if obj.is_editable else ""

	def get_title_suggestion(self, obj):
		content = getattr(self, "_editable_content", "") if obj.is_editable else ""
		if content:
