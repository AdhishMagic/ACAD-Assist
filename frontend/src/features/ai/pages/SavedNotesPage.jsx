...
import { useAISavedNotes } from "../hooks/useAIChat";

export default function SavedNotesPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const { data: notes, isLoading } = useAISavedNotes();
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2">Saved AI Notes</h1>
          <p className="text-muted-foreground text-lg">
            Review and organize structured notes generated from your study sessions.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 min-h-[50vh]">
          ...
          <p className="text-muted-foreground text-lg font-medium animate-pulse">Loading your notes...</p>
        </div>
        ...
        >
          ...
          <h3 className="text-2xl font-bold mb-3 text-foreground">No notes found</h3>
          <p className="text-muted-foreground text-lg max-w-sm">
            {searchQuery ? "Try adjusting your search query or filters." : "You haven't saved any AI study notes yet. Interact with the AI Assistant to generate some."}
          </p>
        </motion.div>
      )}
    </div>
