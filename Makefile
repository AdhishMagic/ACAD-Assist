...
dev-ai: ## Start AI service dev server
	cd ai-layer && uvicorn api.inference.main:app --reload --port 8001

dev-rag: ## Start RAG service dev server
	cd ai-layer && uvicorn api.rag.main:app --reload --port 8002
...
test: ## Run all tests
	...
	cd ai-layer && python -m pytest tests/inference
	@echo "Running RAG service tests..."
	cd ai-layer && python -m pytest tests/rag
...
test-ai: ## Run AI service tests
	cd ai-layer && python -m pytest tests/inference -v

test-rag: ## Run RAG service tests
	cd ai-layer && python -m pytest tests/rag -v
...
lint: ## Run all linters
	...
	cd ai-layer && black --check . && flake8 . && isort --check-only .
	@echo "Linting JavaScript..."
	cd frontend && npm run lint

lint-fix: ## Auto-fix lint issues
	cd backend && black . && isort .
	cd ai-layer && black . && isort .
